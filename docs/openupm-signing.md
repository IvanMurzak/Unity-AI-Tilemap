...
> placeholders (`com.IvanMurzak.Unity.MCP.Tilemap`, `IvanMurzak/Unity-AI-Tilemap`). When you adopt the
...
`com.IvanMurzak.Unity.MCP.Tilemap` package so the warning no longer appears in Unity 6.3+.
...
`Unity-Package/Packages/com.ivanmurzak.unity.mcp.tilemap/` with Unity's UPM CLI, verifies the resulting archive
contains `package/.attestation.p7m` and that its basename begins with
`com.IvanMurzak.Unity.MCP.Tilemap-`, and uploads the signed `.tgz` as a `signed-upm-package`
workflow artifact.
...
```bash
gh secret set UPM_SERVICE_ACCOUNT_KEY_ID     --repo IvanMurzak/Unity-AI-Tilemap
gh secret set UPM_SERVICE_ACCOUNT_KEY_SECRET --repo IvanMurzak/Unity-AI-Tilemap
gh secret set UPM_ORG_ID                     --repo IvanMurzak/Unity-AI-Tilemap
```

### 3. File the OpenUPM listing change

If OpenUPM's package listing for `com.IvanMurzak.Unity.MCP.Tilemap` has `trackingMode: git`,
...
repository at `data/packages/com.IvanMurzak.Unity.MCP.Tilemap.yml`. Open a PR there changing:
...
1. Go to the release page for the new version and confirm a
   `com.IvanMurzak.Unity.MCP.Tilemap-<version>.tgz` asset is attached alongside the
...
2. Inspect the tarball locally to confirm it contains the signing attestation:
   ...
   curl -fsSL -o package.tgz \
     https://github.com/IvanMurzak/Unity-AI-Tilemap/releases/download/<version>/com.IvanMurzak.Unity.MCP.Tilemap-<version>.tgz
